...
- **السر الوحيد في `localStorage`:** `stockaudit_worker_secret`.
  (و`stockaudit_flow_token` لسه موجود — 🔴 معلّق تحت.)
...
آخر مطابقة: 12-09-2026 · `index.js` v5.0.0 · `index.html` v5.0.0
🔴 معلّقة: **Flow Companion** (تحت)

## مسائل مفتوحة

### 🔴 Flow Companion — نداء كتابة لسه خارج الـ Worker

`doFulfillOrders()` في `index.html` بينادي `flow-companion.mivicle.app`
**من المتصفح مباشرةً**، والتوكن في `localStorage`، و**مفيش صف D1** للفعل ده.

اللي اتصلّح في 12-09-2026: التهريب (`esc`) · لقطة وقفل أثناء الدفعة · تقدّم
حقيقي · الدليل من جسم الرد مش `res.ok` لوحدها · نافذة ملخّص بالفشل وسببه.

اللي **لسه مفتوح** ومحتاج قرار أحمد:

1. النقل للـ Worker (التوكن يبقى Secret + النداء من السيرفر) — محتاج قيمة
   `type` جديدة **تتسجّل في `ecommoda-constants` §7 الأول** (القاعدة ٧ في
   `worker-builder`: ممنوع `writeLog` بقيمة مش مسجّلة). القيمة المقترحة:
   `fulfill`.
2. أو **شيل الميزة** لو مابقتش مستخدمة — وساعتها يتشال `LS_FLOW_KEY` وحقل
   الإعدادات معاها.

> لحد ما ده يتقرر، الفعل ده **بيغيّر حالة أوردرات حقيقية وما بيسيبش أي أثر
> في السجل**.
