...
| `get_bulk_index` | **جماعي** — فهرس كل المتغيّرات الصالحة للجرد (باركود · SKU · الرصيد على `LOCATION_ID` · آخر جرد) + `dupBarcodes` + `truncated` + السقوف |
| `bulk_shelf_check` | **جماعي** — المغلَّف مقابل اللي على الرف لكل متغيّر (POST) — استعلام لكل **منتج** مش لكل مقاس |
...
🔴 **عشان كده أي صنف `committed > 0` بيفضل «محتاج مراجعة» ومش بيتنفّذ** لحد ما
`bulk_shelf_check` يجيب رقم المغلَّف الحقيقي. من غير الخطوة دي كل صنف عليه
أوردرات معلّقة كان هيطلع عجز وهمي.
...
④ مراجعة الأوردرات BULK_SHELF_MAX_BATCH = 30 · الواجهة BULK_SHELF_CHUNK = 8
```

🔴 **السقوف بترجع للواجهة من `get_bulk_index`** (`chunkHint`/`maxBatch`/…) —
...
- **الفهرس بياخد أرقامه من `LOCATION_ID` صراحةً** (`inventoryLevel(locationId:)`)،
  مش `inventoryLevels(first: 1)` زي باقي الأداة. الفرق مالوش أثر في متجر بموقع
  واحد، لكن في الجماعي معناه مئات التعديلات محسوبة على أرقام موقع تاني.
...
آخر مطابقة: 18-09-2026 · `index.js` v6.0.0 · `index.html` v6.0.0
🔴 معلّقة: — لا شيء
